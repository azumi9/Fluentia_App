...
async def translate_with_example_gemini(text_to_translate: str, user_level: str | None = None) -> str:
    ...
    prompt = (
        ...
        f"🚨 ВАЖЛИВЕ ПРАВИЛО ЩОДО ОПИСОК ТА БЕЗГЛУЗДОГО ТЕКСТУ 🚨\n"
        f"Якщо користувач ввів випадковий набір літер (наприклад 'цушк', 'авпвап', 'ghjbd'), очевидну незрозумілу описку або неіснуюче слово:\n"
        f"- СУВОРО ЗАБОРОНЕНО намагатися це перекласти, вгадати значення або придумувати приклади.\n"
        f"- Ти маєш проігнорувати всі наступні кроки і видати ЛИШЕ ОДИН рядок точно в такому форматі:\n"
        f"📖 Переклад: 🤔 Здається, тут якась описка або неіснуюче слово. Напиши, будь ласка, ще раз!\n\n"
        f"--- ЯКЩО ТЕКСТ АДЕКВАТНИЙ, ВИКОНАЙ НАСТУПНІ КРОКИ ---\n"
        ...
        f"   - Природний регістр: використовуй слово/фразу ('{text_to_translate}') з правильної літери (великої чи малої) відповідно до правил граматики речення. Не змушуй слово бути з великої літери посеред речення, якщо цього не вимагають правила.\n"
